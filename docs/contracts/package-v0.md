...
`requirements` inventories every executable requirement of the package: its IRI, version, kind (`semantic` or `quantitative`), and the source artifact declaring it. The inventory is derived from, not asserted over, the canonical graph: each entry must resolve to an RDF requirement definition typed `s2s:SemanticRequirement` or `s2s:QuantitativeRequirement` matching the declared kind, carrying the declared `s2s:requirementVersion`, owned by its declared source, and attached to exactly one declared methodology reference through `s2s:hasRequirement` — applicability requirements are methodology-owned, not standard-owned or shape identities. A requirement definition present in the graph but absent from the inventory is rejected, so omitting a requirement and its vectors cannot pass. Every conformance vector names the declared requirement it exercises, and every declared requirement must be exercised by at least three vectors covering both expected outcomes — the manifest-level form of the rule that every executable requirement has mandatory valid, invalid, and boundary test vectors.
...
- `manifest.requirement.kind_invalid` / `.requirement.duplicate` — a requirement entry does not use a known kind or a unique identity;
